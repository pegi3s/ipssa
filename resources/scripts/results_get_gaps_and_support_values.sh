...
TMP_DIR=${WORKING_DIR}/tmp/${FILENAME}
mkdir -p ${TMP_DIR}

#Get alignment gaps

docker run --rm -v ${HOST_WORKING_DIR}:/data pegi3s/alter -i /data/input/6_aligned/${FILENAME}.aln -o /data/tmp/${FILENAME}/${FILENAME}.fasta -ia -of FASTA -oo Linux -op GENERAL

docker run --rm -v ${HOST_WORKING_DIR}/:/data pegi3s/utilities fasta_remove_line_breaks /data/tmp/${FILENAME}/${FILENAME}.fasta
...
echo $(cat ${TMP_DIR}/${FILENAME}.fasta.tmp3) >> ${WORKING_DIR}/results/tabulated/${ORIGINAL_FILENAME}

#Get low support values

docker run --rm -v ${HOST_WORKING_DIR}:/data pegi3s/alter -i /data/input/10_gapped_alignment/2_gapped_alignment/${FILENAME}.aln -o /data/tmp/${FILENAME}/${FILENAME}.fasta.support -ia -of FASTA -oo Linux -op GENERAL

docker run --rm -v ${HOST_WORKING_DIR}/:/data pegi3s/utilities fasta_remove_line_breaks /data/tmp/${FILENAME}/${FILENAME}.fasta.support
...
echo $(cat ${TMP_DIR}/${FILENAME}.fasta.tmp4.support) >> ${WORKING_DIR}/results/tabulated/${ORIGINAL_FILENAME}
